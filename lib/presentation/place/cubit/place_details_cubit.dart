...
class PlaceDetailCubit extends Cubit<PlaceDetailState> {
  ...
  PlaceDetailCubit(this._repo, String placeId) : super(PlaceDetailLoading()) {
    _load(placeId);
  }
  ...
  Future<void> _load(String placeId) async {
    try {
      // 1. Load the place
      await Future.delayed(const Duration(milliseconds: 300)); // simulated API
      final place = _repo.getPlaceById(placeId);
      if (place == null) {
        emit(
          PlaceDetailError(
            CacheFailure(code: 'not-found', message: 'Place not found'),
          ),
        );
        return;
      }
      ...
      emit(
        PlaceDetailLoaded(
          place: place,
          routeInfo:
              routeInfo ?? RouteInfo(routepoints: [], distance: 0, duration: 0),
          userLocation: location,
        ),
      );
    } catch (e) {
      emit(PlaceDetailError(UnknownFailure()));
    }
  }
}
